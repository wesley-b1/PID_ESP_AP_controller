...
#include <Fonts/FreeSerif9pt7b.h> // <--- necessário para FreeSerif9pt7b
...
static const TickType_t samplingInterval = 10 / portTICK_PERIOD_MS;      // 10ms ou 0.01s
static const TickType_t timeToStartInterval = 8000 / portTICK_PERIOD_MS; // 8s para iniciar as malhas abertas e fechadas
...
static const uint8_t SENSOR_Y = 34; // plantas 5 e 7
...
// VARIÁVEIS PID
float mvAtual = 0; // usada para receber U mudado no PID e receber o degrau
...
//   // limpaDisplay();
// }
void exibeMensagemInicial(String texto, IPAddress ip)
{
  limpaDisplay();

  display.setFont(&FreeSerif9pt7b);
  display.setTextColor(SSD1306_WHITE);
  // display.setTextSize(1);// usando setFont o setTextSize é ignorado

  int16_t x = 0, y = 14; //(coluna, linha): da linha 0 a 15 é amarelo

  if (texto != "") // se a string não for vazia
  {
    display.setCursor(x, y);
    display.println(texto);
    y += 25;
  }
  if (!(ip == IPAddress(0, 0, 0, 0))) // se o IP não for vazio
  {
    display.setCursor(x, y);
    display.print("IP: ");
...
// Callback do Websocket: processar mensagens recebidas via WebSocket
void onWebSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
  ...
  if (type == WStype_TEXT)
  {
    ...
    else if (msg.startsWith("setSp:"))
    {
      ...
      Serial.printf("[WS] Setpoint recebido: %.2f V\n", spVolts);
...
void getSensorReadingCallback(TimerHandle_t xTimer)
{
  ...
  if (!usarPID)
  {
    // Aplica o degrau usando o setpoint enviado pelo websocket
    // float degrau = sp * VCC / RESOLUTION; // converte bits -> volts
    float degrau_bits = sp; // já em bits de 0 a 4096
    float degrau_volts = (degrau_bits * VCC) / RESOLUTION;
    // float degrau = sp;
    if (degrau_volts > VCC)
      degrau_volts = VCC; // saturação por segurança

    uint8_t valorDAC = (uint8_t)(255 * (degrau_volts / VCC));

    if (plantaSelecionada >= 1 && plantaSelecionada <= 4)
      dacWrite(MV_PIN_1, valorDAC);
    else if (plantaSelecionada >= 5 && plantaSelecionada <= 7)
      dacWrite(MV_PIN_2, valorDAC);

    mvAtual = degrau_volts;
  }

  // Depois: leitura do sensor (para refletir ação do degrau)
  if (plantaSelecionada >= 1 && plantaSelecionada <= 4)
    sensorReadingInt = analogRead(SENSOR_X);
  else if (plantaSelecionada >= 5 && plantaSelecionada <= 7)
    sensorReadingInt = analogRead(SENSOR_Y);
  ...
  sensorReadingVoltage = (VCC * sensorReadingInt) / RESOLUTION;
  ...
  if (usarPID)
  {
    ...
    mvAtual = (int)u * VCC / 255.0; // convertendo em tensão
  }
...
//
//     uint8_t degrau = 1.0; // entrada degrau unitária
//     if (degrau >= 3.3)    // limitador para evitar saturação do DAC
//     {
//       degrau = 3.3;
//     }
//     uint8_t valorDAC = (uint8_t)(255 * (degrau / 3.3)); // convertendo em bits de acordo com o VCC
//     if (plantaSelecionada >= 1 && plantaSelecionada <= 4)
//       dacWrite(MV_PIN_1, valorDAC);
//     else if (plantaSelecionada >= 5 && plantaSelecionada <= 7)
//       dacWrite(MV_PIN_2, valorDAC);
...
void setStepInputReadingCallback(TimerHandle_t xTimer)
{
  if (!usarPID)
  {
    ...
    Serial.printf("[MA] Degrau aplicado: %.2f V (bits=%u)\n", degrau_volts, valorDAC);
  }
}
...
void configurarMalhaFechada()
{
  ...
  if (!ganhosFornecidosViaWeb)
  {
    ...
    case 1:
      // Planta 1 1° ordem
      setTunings(0.2042, 2.2691, 0.0); // KP, KI, KD
      break;
    case 2:
      // Planta 2 1° ordem
      setTunings(0.6219, 3.3958, 0.0); // KP, KI, KD
      break;
    case 3:
      // Planta 3 1° ordem
      setTunings(6.2532, 3.4886, 0.0); // KP, KI, KD
      break;
    case 4:
      // Planta 4 1° ordem
      setTunings(5.6813, 8.0834, 0.0); // KP, KI, KD
      break;
    case 5:
      // Planta 1 2° ordem
      // CH-0: 4.7uF (SOBREAMORTECIDO)
      setTunings(0.1048, 0.8811, 0.0102); // KP, KI, KD
      break;
    case 6:
      // Planta 2 2° ordem
      ////CH-2: 227nF (SUBAMORTECIDO)
      setTunings(0.2195, 0.2211, 0.1105); // KP, KI, KD
      break;
    case 7:
      // Planta 3 2° ordem
      // CH-3: 2.2uF (CRITICAMENTE-AMORTECIDO)
      setTunings(1.1500, 0.4115, 0.0905); // KP, KI, KD
      break;
    }
  }
...
void configurarMalhaAberta()
{
  // configurando o MUX necessária
  // se o degrau ocorre antes de o timer executar, a seleção da planta ainda pode estar errada
  configurarMUX(); // cria o timer
  // garante configuração imediata
  setMUXCallback(NULL); // essa função depende da criação do timer muxConfigTimer

  vTaskDelay(pdMS_TO_TICKS(8000)); // Esperar descarga do capacitor por 8s
  // aplica degrau após um tempo (único disparo)
  stepInputStartTimer = xTimerCreate("stepInputStartTimer", timeToStartInterval, pdFALSE, NULL, setStepInputReadingCallback);
  xTimerStart(stepInputStartTimer, 0);

  // cria timer periódico para leitura da PV (igual malha fechada)
  getSensorReadingTimer = xTimerCreate("getSensorReadingTimer", samplingInterval, pdTRUE, NULL, getSensorReadingCallback);
  if (getSensorReadingTimer != NULL)
  {
    LOGI("ControlePID", "Iniciando leituras em malha aberta...");
    xTimerStart(getSensorReadingTimer, 0);
  }
  else
  {
    LOGE("ControlePID", "Erro ao criar getSensorReadingTimer em malha aberta!");
  }
